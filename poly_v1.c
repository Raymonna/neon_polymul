...
typedef int64_t ll;
#define Max 256
const ll Mod = 689522689;//677795329;//689522689;   // 模数
const ll SpMul = 34286;//45903;//34286;  // 原根
...
static int64_t bred(int64_t a, int32_t m, uint32_t mu) {
...
void poly_Rq_mul_small(int16_t *h, const int16_t *f,const int8_t *g)
{
	...
	for(int16_t i = 0; i < 6; i++){
		for(int16_t j = 0; j < 6; j++){
			for(int16_t e = 0; e < len256; e++){
				tmp[j + 6*i][e] = (data1[i][e] * data2[j][e]) % Mod;
			}
		}
	}
	...
	for(int16_t i = 0; i < 6; i++){
		for(int16_t j = 0; j < 6; j++){
			for(int16_t e = 0; e < len256; e++){
				tmp_h[((i + j)<<7) + e] = tmp[(i<<2) + (i<<1) + j][e] % NTRUP_Q;//bred(tmp[(i<<2) + (i<<1) + j][e], NTRUP_Q, mu); 
			}
		}	
	}
	for(int i = 0; i < 10; i++){
	...
	for (int i = 2 * NTRUP_P - 2; i >= NTRUP_P; i--) {
      h[i - NTRUP_P] = (tmp_h[i - NTRUP_P] + tmp_h[i]) % NTRUP_Q;//bred_smallm(tmp_h[i - NTRUP_P] + tmp_h[i], NTRUP_Q, mu);
